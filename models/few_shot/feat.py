...
class FEAT(FewShotModel):
    ...
    def _forward(self, support, query, prefix):
        ...
        # support.shape: [meta_batch_size x shot x way x feat_dim]
        # query.shape:   [meta_batch_size x query x way x feat_dim]
        cur_way = eval(f'self.args.{prefix}way')
        cur_shot = eval(f'self.args.{prefix}shot')
        cur_query = eval(f'self.args.{prefix}query')

        support = support.reshape(self.args.meta_batch_size, cur_shot, cur_way, *support.shape[2:])
        query   = query.reshape(self.args.meta_batch_size, cur_query, cur_way, *query.shape[2:])

        # get mean of the support
        proto = support.mean(dim=1) # Ntask x NK x d
        num_batch = proto.shape[0]
        num_proto = proto.shape[1]
        num_query = cur_query * cur_way
        ...
        # for regularization
        if self.training:
            aux_task = torch.cat([support.view(1, cur_shot, cur_way, emb_dim), 
                                  query.view(1, cur_query, cur_way, emb_dim)], 1) # T x (K+Kq) x N x d
            num_query = np.prod(aux_task.shape[1:3])
            aux_task = aux_task.permute([0, 2, 1, 3])
            aux_task = aux_task.contiguous().view(-1, cur_shot + cur_query, emb_dim)
            # apply the transformation over the Aug Task
            aux_emb = self.slf_attn(aux_task, aux_task, aux_task) # T x N x (K+Kq) x d
            # compute class mean
            aux_emb = aux_emb.view(num_batch, cur_way, cur_shot + cur_query, emb_dim)
