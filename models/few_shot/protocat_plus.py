...
class ProtoCAT_Plus(RGMultiModalProtoNetPlus):
    def __init__(self, args, num_classes):
        ...
        if not self.init_prototypes:
            ...
            from models.few_shot.tima_plus import Transformer
            self.trans_1 = Transformer(n_trg_vocab=2048, d_model=2048, d_inner=1024, n_layers=1, n_head=4, d_k=512, d_v=1024)
    ...
    def _forward(self, supports, queries, y_total, prefix, way=None, shot=None):
        ...
        cur_prototypes[mdl3] = self.trans_1(src_seq=cur_prototypes[mdl2].view(1, -1, self.hdim), trg_seq=self.share_prototypes_video.view(1, -1, self.hdim))
        cur_prototypes[mdl3] = cur_prototypes[mdl3].view(*self.share_prototypes_video.shape)
    ...
    def _forward_gfsl(self, supports, queries, queries_seen, support_label, seen_label, y, prefix, way=None, shot=None):
        ...
        prototypes[mdl3] = self.trans_1(src_seq=prototypes[mdl2].view(1, -1, self.hdim), trg_seq=prototypes[mdl1].view(1, -1, self.hdim))
        prototypes[mdl3] = prototypes[mdl3].view(prototypes[mdl1].shape)
