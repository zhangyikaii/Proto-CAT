...
class MultiModalTIMAPlus(MultiModalFewShotModel):
    def __init__(self, args, num_classes):
        super().__init__(args)
        # self.hdim = 2048
        # # n_trg_vocab 最后一层输出dim, 
        # self.tima_1 = Transformer(n_trg_vocab=2048, d_model=2048, d_inner=1024, n_layers=1, n_head=4, d_k=512, d_v=1024)
        # self.tima_2 = Transformer(n_trg_vocab=2048, d_model=2048, d_inner=1024, n_layers=1, n_head=4, d_k=512, d_v=1024)
        # from models.few_shot.tima import MultiHeadAttention as MHA
        # self.slf_attn1 = MHA(n_head=1, d_model=self.hdim, d_k=self.hdim, d_v=self.hdim, dropout=0.3)
        # self.slf_attn2 = MHA(n_head=1, d_model=self.hdim, d_k=self.hdim, d_v=self.hdim, dropout=0.3)
